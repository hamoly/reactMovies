...
class App extends Component {
	...
	fetchMovies = () => {
		axios
			.get(`${process.env.REACT_APP_MOVIE_API_KEY}${this.state.pageNumber}`)
			.then((result) => {
				this.setState({
	...
	addToLiked = (e) => {
		...
		if (this.likedMovieState(id)) {
			this.setState({
				liked: liked.filter((liked) => {
					return liked.id !== parseInt(id);
				}),
			});
		} else {
				...
				<div className='container-fluid'>
					...
					<div className='mt-115' id='movies'>
						{display === 'login' || display === 'signup' ? (
							<Login
								display={display}
								setStateUserForm={this.setStateUserForm}
						  ...
						  query !== '' ? (
							...
							<MovieListRenderer
								displayLikedList={this.displayLikedList}
								display={display}
								likedMovieState={this.likedMovieState}
								user={user}
								movies={renderedMovieList}
								handleFav={this.addToLiked}
								error={error}
								isLoaded={isLoaded}
							/>
						)}
